...
# ╔═╡ 5c467926-ac37-11f1-87fc-bd159fe1bf9b
begin
	using UnfoldBIDS
	using LazyArtifacts
	using BSplineKit
...
# ╔═╡ 94c886cf-07a5-4207-a473-af538b1d3268
plots_dir = "/store/users/schepers/projects/FreeViewingMSCOCO/plots/2026-09-16_Meeting_Bene/"

# ╔═╡ 9bc9f7a6-ebd0-4ebc-b417-5746e53afe98
md"""
# Load Unfold models of all subjects
"""

# ╔═╡ 019304f9-8ff4-48a0-ae15-90ee6ed38a41
begin
	bids_root = "/scratch/data/2024FreeViewingMSCOCO"
	predictor = :sacc_visual_angle_w
	event = "fixation"
	derivatives_subfolder = joinpath("erp-analysis",string(event), string(predictor))
end

# ╔═╡ 4980604a-2665-45cf-9a8e-5bc590effae6
model_df = load_results(bids_root, derivatives_subfolder = derivatives_subfolder)

# ╔═╡ e8be5d03-b41b-4b34-938a-0ef8cbed550e
md"""
# Calculate marginal effects
"""

# ╔═╡ 5e46272f-152c-4d70-963b-c918c337e2cf
begin
	if predictor == :sacc_duration_w
		effects_dict = Dict(:sacc_duration_w =>range(20,59,step=5)/1000) # TODO: Adapt
		plotting_dict = Dict(:sacc_duration_w => range(20,59,step=5)[1:2:end]/1000)
	elseif predictor == :sacc_visual_angle_w
		# [1.04, 10.5154]
		effects_dict = Dict(:sacc_visual_angle_w => append!([1.04],range(2,10,9)))
		frp_dict = Dict(:sacc_visual_angle_w => effects_dict[:sacc_visual_angle_w][1:2:end])
		topo_dict = Dict(:sacc_visual_angle_w => effects_dict[:sacc_visual_angle_w][1:3:end])
	end
	
end

# ╔═╡ 1e119d82-fb2e-4510-aebe-6c242e0cad58
eff = bids_effects(model_df, effects_dict)

# ╔═╡ bafa1c13-ad75-48e1-b99a-e535c2acd0ff
# Calculate mean over subjects
mean_eff = combine(groupby(eff, [:eventname,predictor,:channel,:time]), :yhat => mean)

# ╔═╡ fb9d8c78-0962-40fa-9e0b-eb2f7dc8547d
md"""
# Load channel idx-name mapping file
"""

# ╔═╡ 35808c2c-27f3-424d-afec-2c80f66de7af
# TODO: Check whether it's the same for all subjects
begin
	sub = "005"
	ch_mapping_path = joinpath(bids_root,"derivatives",derivatives_subfolder,"sub-$(sub)","eeg","sub-$(sub)_channel_mapping.tsv")
	ch_mapping_df = CSV.read(
		ch_mapping_path,
		DataFrame,
		delim="\t",
	)
...
# ╔═╡ b4062962-3904-47f3-8ef5-b87a3852aeaa
begin
	ch_name = "POz"
	ch_idx = only(ch_mapping_df[ch_mapping_df.ch_name .== ch_name,:ch_idx])
end

# ╔═╡ 21246585-9267-492e-99f3-4414d6d06e24
md"""
# Extract positions and project to 2D
"""

# ╔═╡ 57462f6e-f805-49eb-b08c-f582e10747e4
function parse_vector(s::String)
    # Remove brackets and split by commas
    stripped = strip(s, ['[', ']'])
    parts = split(stripped, ",")
    # Convert each part to Float64
    return [parse(Float64, strip(part)) for part in parts]
end

# ╔═╡ ba4b7e24-f6f2-4c40-95ad-2131daf9a85a
positions_2d = to_positions(reduce(hcat, parse_vector.(ch_mapping_df.ch_position)))

# ╔═╡ e3f90168-d4ab-47cc-a49d-6574426eb231
md"""
# Lambda peak detection
"""

# ╔═╡ 766204fd-ccbd-40e9-b97e-746859883da3
function find_max_peak(t,y)
	
	peak_info=  findmaxima(y) |> peakproms!() |> peakwidths!()
	...
	max_peak = argmax(heights)
	max_peak_idx = indices[max_peak]

	peak_latency = t[max_peak_idx]
	peak_amplitude = heights[max_peak]
	peak_edge_idx = peak_info.edges[max_peak][1]
	peak_edge = t[Int64(round(peak_edge_idx))]

	return peak_latency, peak_amplitude, peak_edge

end

# ╔═╡ 713be176-0471-42c3-94f8-e40098367b77
function detect_peak(eff_df, predictors, estimate_col, time_window::Union{Nothing, Tuple}=nothing)
	
	if !isnothing(time_window)
		eff_df = filter(r -> (time_window[1] <= r.time <= time_window[2]), eff_df)
	end

	grouping_options = ["subject", "eventname", "channel"]
	grouping_factors = String[]

	for option in grouping_options
		if option in names(eff_df)
			push!(grouping_factors, option)
		end
	end
	push!(grouping_factors, predictors...)
	
	eff_df_grouped = groupby(eff_df, grouping_factors)
	
	peak_df = combine(eff_df_grouped, [:time, estimate_col] => ((t,y) -> begin
    peak_latency, peak_amplitude, peak_edge = find_max_peak(t,y)
    (peak_latency=peak_latency, peak_amplitude=peak_amplitude, peak_edge=peak_edge)
end) => AsTable)
end

# ╔═╡ a9bc8234-ab71-4d1a-a54d-481cb358dbe3
peak_df_mean = detect_peak(mean_eff,[string(predictor)], :yhat_mean, (0.05,0.25))

# ╔═╡ 69980f04-d0d8-427a-b5fe-6e3ce594129d
peak_df_all_subjects = detect_peak(eff,[string(predictor)], :yhat, (0.05,0.2))

# ╔═╡ 6765e88a-da48-42fd-9524-2f9abd209bc3
md"""
# Channels of interest
"""

# ╔═╡ 3f61697b-211a-49e9-9869-b94e5f95bc50
begin
	ch_idx_roi = [29,30,31,53,54,91,120,121,123,124]
	ch_name_roi = ch_mapping_df[in.(ch_mapping_df.ch_idx,Ref(ch_idx_roi)),:ch_name]
end

# ╔═╡ 827b3e75-1f53-4c80-81da-1ba048833ef6
begin
	using TopoPlots

	colormap = vcat(1,colorschemes[:tab10].colors[1:length(ch_idx_roi)])
	
	marker_list = zeros(length(positions_2d))
	marker_list[ch_idx_roi] .= 1:length(ch_idx_roi)

	labels = repeat([""], length(positions_2d))
	labels[ch_idx_roi] = ch_name_roi

	stroke_color = repeat([:white], length(positions_2d))
	stroke_color[ch_idx_roi] .= :black
	
	TopoPlots.eeg_topoplot(
		marker_list,
		#labels = labels,
		positions=positions_2d,
		label_scatter = (markersize = 25, strokecolor = stroke_color, strokewidth = 1),
...
# ╔═╡ 15ed76c0-22b1-421a-a52d-02fb8f0383aa
md"""
# Lambda response figure
"""

# ╔═╡ 3bdc8569-cd34-4790-a69b-f7f63b315b0e
let
	# Figure layout
	f = Figure(size = (1200,800), fontsize=19)
	g_abc = f[1, 1:3] = GridLayout()
	g_b = g_abc[1, 2] = GridLayout()
	g_c = g_abc[1, 3] = GridLayout()
	g_d = f[2, 1:3] = GridLayout()

	# FRP plot (POz) (Panel A)
	eff_sub_FRP = filter(r -> (r.eventname == string(event)) && (r.channel == ch_idx) && (r[predictor] in frp_dict[predictor]), mean_eff)
	
	erp = plot_erp!(
		g_abc[1,1],
	    eff_sub_FRP;
	    mapping = (; y = :yhat_mean, color = predictor, group = predictor),
		colorbar = ( ;label = "Saccade amplitude [°]", labelrotation = pi/2),
		visual = (; linewidth=3, colormap=:roma, color=:roma),
		axis = (; xlabel = "Time [s]"),
	)

	# Plot detected peaks
	peak_df_mean_frp = filter(r -> r.channel == ch_idx && r.eventname == string(event) && (r[predictor] in frp_dict[predictor]),peak_df_mean)

	scatter!(
		current_axis(),
		peak_df_mean_frp.peak_latency,
		peak_df_mean_frp.peak_amplitude,
		color = :black,
	)

	# Plot zero lines for visual orientation
	vlines!(current_axis(),[0], color=:black, linestyle = :dash)
	hlines!(current_axis(),[0], color=:black, linestyle = :dash)

	text!(current_axis(), 1, 1, space=:relative, text = ch_name, align = (:right, :top), offset = (-20,-20), fontsize=20, font=:bold)
	
	xlims!(current_axis(),-0.2,0.6)

	# Lambda amplitude effect plot (POz) (Panel B)
	peak_df_mean_sub = filter(r -> r.channel == ch_idx && r.eventname == string(event) && (r[predictor] in effects_dict[predictor]),peak_df_mean)
	...
	# Subject lines
	peak_df_all_subjects_sub = filter(r -> r.channel == ch_idx && r.eventname == string(event) && (r[predictor] in effects_dict[predictor]),peak_df_all_subjects)

	subjects = unique(peak_df_all_subjects_sub.subject)
	for (i, subject) in enumerate(subjects)
		peak_df_single_subject = filter(r -> r.subject == subject, peak_df_all_subjects_sub)
		lines!(
			ax_b,
			Float64.(peak_df_single_subject[!,predictor]),
			Float64.(peak_df_single_subject.peak_amplitude),
			color = :gray,
		)
	end
	...
	lae_plot = lines!(
		ax_b,
		peak_df_mean_sub[!,predictor],
		peak_df_mean_sub.peak_amplitude,
		linewidth=4,
		color=peak_df_mean_sub.peak_amplitude,  # Use the y-values for color
    	colormap=:viridis,      # Same colormap as heatmap
        #colorrange=(minimum(amplitude), maximum(amplitude)),
	)
	scatter!(
		ax_b,
		peak_df_mean_sub[!,predictor],
	...
	ax_b.ylabel = "Peak amplitude [μV]"
	ax_b.xlabel = "Saccade amplitude [°]"

	text!(ax_b, 1, 0, space=:relative, text = ch_name, align = (:right, :bottom), offset = (-20,20), fontsize=20, font=:bold)

	Colorbar(g_b[1, 2], lae_plot, label = "Voltage [μV]")

	# Lambda amplitude effect plot (occipital channels) (Panel C)
    ax_c = Axis(
		g_c[1,1],
		#aspect = 1,
		xlabel = "Saccade amplitude [°]",
		ylabel = "Relative peak amplitude [μV]",
		ygridvisible=false,
		xgridvisible=false,
	)
	hidespines!(ax_c, :t, :r)
	
	peak_df_mean_roi = filter(r -> (r.channel in ch_idx_roi) && (r.eventname == string(event)) , peak_df_mean)
	for (i,ch) in enumerate(ch_idx_roi)
		peak_df_mean_roi_ch = filter(r -> r.channel == ch, peak_df_mean_roi)
		lines!(
			ax_c,
			Float64.(peak_df_mean_roi_ch[!,predictor]),
			Float64.(peak_df_mean_roi_ch.peak_amplitude).-Float64.(peak_df_mean_roi_ch.peak_amplitude[1]),
			label = ch_name_roi[i],
			color = colorschemes[:tab10].colors[i], 
			linewidth=2.5
		)
	end
	g_c[1,2] = Legend(
	...
	TopoPlots.eeg_topoplot!(
		...
		#labels = labels,
		positions=positions_2d,
		label_scatter = (markersize = 4.85, strokecolor = stroke_color, strokewidth = 0.5),
		interpolation = NullInterpolator(),
		head = (color = :black, linewidth = 0.5),
		#axis=(aspect=DataAspect(),),
		colormap = colormap,
		#label_text=true
	)
	...
	# Topoplot series (Panel D)
	bin_width = 0.025

	eff_sub_topo = filter(r -> (r.eventname == event) && (r[predictor] ∈ topo_dict[predictor]) && (0 <= r.time <= 0.25), mean_eff)
	...
	plot_topoplotseries!(
		...
		bin_width = bin_width,
	    positions = positions_2d,
	    axis = (; xlabel = "Time windows [s]"), #ylabel = "Saccade amplitude [°]"),
		mapping = (; y = :yhat_mean, row = :sacc_visual_angle_w),
		visual = (; contours = (; color = :black)),
	)

	# Add panel labels
	for (label, layout) in zip(["A", "B", "C", "D"], [g_abc[1,1], g_b, g_c, g_d])
	...
	#colsize!(g_abc, 2, Aspect(1,1.0))
	colsize!(g_abc, 2, Auto(0.9))
	#colsize!(g_abc, 3, Relative(0.3))
	colsize!(g_c, 1, Relative(0.8))

	#resize_to_layout!(f)
	save(joinpath(plots_dir,"sacc_amp_effect_lambda_response.svg"), f)
	current_figure()
end

# ╔═╡ 16ad32d8-f331-4ba4-b488-eb690535fce2
md"""
## Helper functions
"""
...
manifest_format = "2.0"
project_hash = "0b9b6e45aeb96cda760208a19060e4769d432f40"
...
git-tree-sha1 = "86e8a58f896ff413b02629958364d21f39ab925d"
...
git-tree-sha1 = "a7341cb28382499457099dbfb2c064d9c03ddfba"
...
git-tree-sha1 = "6b34cdfa6b08d2a8c5a50338b10483c0602633d5"
...
git-tree-sha1 = "47c04e48a357741895907f594ec569fbd651cdba"
...
[[deps.Unfold]]
deps = ["DSP", "DataFrames", "Distributions", "DocStringExtensions", "Effects", "FileIO", "GLM", "ImageTransformations", "Interpolations", "IterativeSolvers", "JLD2", "LinearAlgebra", "Logging", "MLBase", "MLUtils", "Missings", "OrderedCollections", "PooledArrays", "PrecompileTools", "ProgressMeter", "Random", "SimpleTraits", "SparseArrays", "StaticArrays", "Statistics", "StatsAPI", "StatsBase", "StatsFuns", "StatsModels", "Suppressor", "Tables", "Term", "TimerOutputs", "TypedTables"]
git-tree-sha1 = "4213593f18c0328f689277fa835135a6f512b874"
...
[[deps.UnfoldBIDS]]
deps = ["Artifacts", "CSV", "Continuables", "DataFrames", "DataFramesMeta", "LazyArtifacts", "Printf", "ProgressBars", "Statistics", "StatsModels", "Unfold"]
git-tree-sha1 = "16c3f8c62c8b6f7f23d84ec0a589ec840044b162"
uuid = "b54e767b-1ebd-4480-ac2a-f5f4d8853074"
version = "0.4.2"

    [deps.UnfoldBIDS.extensions]
    MNEext = "PyMNE"
    UnicodePlotsExt = ["UnicodePlots", "Term"]

    [deps.UnfoldBIDS.weakdeps]
...
git-tree-sha1 = "64dac8e4b910cc8e4471dd218abd91dac5570056"
